...
*/
  ...
  */
  ...
  // function cleanCPF(cpf) {
  //   let cpfArr = [];
  //   cpf.replace(/([\d])/g, function (regex, cpfNumbers) {
  //     return cpfArr.push(cpfNumbers)
  //   })
  //   return cpfArr.join('')
  // }
  ...
  */
  ...
  // ?
  // let result1 = cleanCPF("049-214 3421-1")
  // let reducedCpf = result1.reduce(function (acumulado, atual, index) {
  //   let separator = index === result1.length - 1 ? '-' : '.';
  //   return acumulado + atual + separator
  // })
  // console.log(reducedCpf)
  ...
  ["junho", "julho"]
  */
  let stringToCheck = "Os meses de janeiro, junho e julho começam com a letra j."
  let check = stringToCheck.match(/(junho|julho)/g)

  // function check(stringToCheck) {
  //   let result = [];
  //   let checking = stringToCheck.match(/junho|julho/)
  //   if (checking) {
  ...
  */
  let htmlToCheck = "<div><ul><li></li><li></li><li><span></span></li></ul></div>";
  let resultHtml = htmlToCheck.match(/\<\w+><\/\w+>/g);
  ...
  */
  let textToReplace = "<h1>Título da página</h1><p>Este é um parágrafo</p><footer>Rodapé</footer>";
  let result = textToReplace.replace(/<(\w+)>([^>]+)<\/\w+>/g, `<$1>O texto dentro da tag $1 é $2</$1>\n`);
  console.log(result)
  // function resulting() {
  //   let texts = textToReplace.match(/(\>)(.*?)(\<)/g).map(function (a) {
  //     a.replace(/(\>)(.*?)(\<)/g[0], `$1''$3''`)
  //     // return a.replace('<', '').replace('>', '');
  //   }).filter((text) => text !== '');

  //   let tags = textToReplace.match(/(<)(\w+)(>)/g);
  //   tags.forEach(function (a) {
  //     console.log('o texto dentro da tag ' + a + ' é ')
  //   })
  //   console.log(tags)
  //   console.log(texts)
